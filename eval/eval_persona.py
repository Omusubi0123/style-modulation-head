...
from src.visualize import ProjectionTracker, ProjectionVisualizer
...
def sample_steering(
    ...
    """
    tracker = ProjectionTracker(model, tokenizer, vector, layer, steering_type)
...
def sample(
    ...
    for i, messages in enumerate(conversations):
        try:
            ...
            # チャットテンプレートが利用できない場合のフォールバック
            print(f"Warning: Chat template not available, using fallback format: {e}")
            # シンプルなプロンプト形式を使用
            if len(messages) == 1:
                # 単一メッセージの場合
                text = messages[0]["content"]
            else:
                # 複数メッセージの場合、最後のユーザーメッセージを取得
                user_messages = [
...
async def eval_batched(
    ...
    if coef != 0:
        ...
        if track_projections:
            ...
            prompts, answers = result
    elif track_projections and vector is not None and layer is not None:
        # coef=0でも投影追跡が有効な場合は投影のみ計算
        tracker = ProjectionTracker(llm, tokenizer, vector, layer, steering_type)
        prompts, answers, all_projections_data = tracker.generate_with_projections(
            all_conversations,
            coef=0.0,
            bs=batch_size,
            temperature=questions[0].temperature,
